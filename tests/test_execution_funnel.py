...
def test_rejection_reasons_are_percentages_of_total_rejected_not_detected():
    report = build_execution_funnel_report(
        detected=1000, profitable_after_fees=400, rejected=300, executable=50, executed=40, closed=30,
...
def test_zero_detected_never_divides_by_zero():
    report = build_execution_funnel_report(
        detected=0, profitable_after_fees=0, rejected=0, executable=0, executed=0, closed=0, rejection_counts=[]
    )
...
def test_executable_plus_rejected_equals_detected_when_no_reason_is_missing():
    ...
    report = build_execution_funnel_report(
        detected=500, profitable_after_fees=200, rejected=450, executable=50, executed=45, closed=40,
...
def test_stage_lookup_returns_none_for_an_unknown_name():
    report = build_execution_funnel_report(
        detected=10, profitable_after_fees=5, rejected=5, executable=5, executed=3, closed=2, rejection_counts=[]
    )
