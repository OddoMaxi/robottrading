...
from app.shadow.decision import decisions_agree, evaluate_shadow_decision
...
def test_evaluate_shadow_decision_allocates_when_capital_and_ev_are_fine():
    ...
    decision = evaluate_shadow_decision(_opp(), ledger, now)
...
def test_evaluate_shadow_decision_rejects_negative_ev():
    ledger = ShadowCapitalLedger(total_capital_usd=10_000.0)
    now = datetime.now(UTC).replace(tzinfo=None)
    decision = evaluate_shadow_decision(_opp(expected_profit_usd=-2.0), ledger, now)
...
def test_evaluate_shadow_decision_rejects_missing_data():
    ledger = ShadowCapitalLedger(total_capital_usd=10_000.0)
    now = datetime.now(UTC).replace(tzinfo=None)
    decision = evaluate_shadow_decision(_opp(expected_profit_usd=None, capital_velocity_score=None), ledger, now)
...
def test_evaluate_shadow_decision_sizes_down_to_whatever_capital_is_available():
    ...
    now = datetime.now(UTC).replace(tzinfo=None)
    decision = evaluate_shadow_decision(_opp(capital_usd=1_000.0, expected_profit_usd=5.0), ledger, now)
    assert decision.outcome == MasterOutcome.ALLOCATE
...
def test_evaluate_shadow_decision_rejects_when_pool_is_already_fully_locked():
    ...
    decision = evaluate_shadow_decision(_opp(capital_usd=1_000.0), ledger, now)
...
def test_evaluate_shadow_decision_two_opportunities_same_instant_genuinely_compete():
    ...
    pool at the same instant — only one can be allocated."""
    ledger = ShadowCapitalLedger(total_capital_usd=5_000.0)
    now = datetime.now(UTC).replace(tzinfo=None)
    cex_opp = _opp(engine=Engine.CEX, capital_usd=5_000.0, expected_profit_usd=10.0)
    dex_opp = _opp(engine=Engine.DEX, capital_usd=5_000.0, expected_profit_usd=10.0)

    first = evaluate_shadow_decision(cex_opp, ledger, now)
    second = evaluate_shadow_decision(dex_opp, ledger, now)
